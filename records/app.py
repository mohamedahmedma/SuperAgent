...
app = FastAPI(
    ...
    description=(
        ...
        "**Two credentials are required for every parent-facing read.** The `X-API-Key` "
        "header proves which *system* is calling; the `guardian_id` in the path names the "
        "*person* on whose behalf it asks. A key alone grants nothing — the permitted "
        "student set is resolved server-side from the guardian link table on every request "
